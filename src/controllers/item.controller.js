...
export async function updateProduct(req, res, next) {
  try {
    ...
    if (!isOwner && !isAdmin) return res.status(403).json({ error: 'Sin permiso para editar este artículo' });
...
export async function deleteProduct(req, res, next) {
  try {
    ...
    const isOwner = item.fk_seller_id === req.user.id_users;
    const isAdmin = req.user.role === 'administrator';
    if (!isOwner && !isAdmin) return res.status(403).json({ error: 'Sin permiso para eliminar este artículo' });
...
export async function uploadImages(req, res, next) {
  try {
    ...
    const existingPhotos = await ItemModel.getPhotos(id);
    if (existingPhotos.length + req.files.length > 5) {
      return res.status(400).json({ error: `Solo se permiten hasta 5 imágenes por artículo (ya tiene ${existingPhotos.length})` });
    }

    // Guardar en uploads/ (carpeta local)
...
export async function publishProduct(req, res, next) {
  try {
    ...
    const published = await ItemModel.publishItem(id);
    res.json(published);
  } catch (err) {
    next(err);
  }
...
export async function soldProduct(req, res, next) {
  try {
    ...
    const sold = await ItemModel.markAsSold(id);
    res.json(sold);
  } catch (err) {
    next(err);
  }
