...
export async function findOrCreate({ fk_items_id, fk_seller_id, fk_buyer_id }) {
  ...
  const [existing] = await pool.query(
    `SELECT * FROM conversations
     WHERE fk_items_id = ? AND fk_seller_id = ? AND fk_buyer_id = ?
     LIMIT 1`,
  ...
  const [rows] = await pool.query(
    'SELECT * FROM conversations WHERE id_conversations = ? LIMIT 1',
    [result.insertId]
  );
  ...
  const [rows] = await pool.query(
    `SELECT c.*,
            ...
            (SELECT COUNT(*) FROM messages m
             WHERE m.fk_conversations_id = c.id_conversations
               AND m.fk_users_id_received = ? AND m.read = false) AS unread_count
     FROM conversations c
     JOIN items i ON i.id_items = c.fk_items_id
     JOIN users seller ON seller.id_users = c.fk_seller_id
     JOIN users buyer  ON buyer.id_users  = c.fk_buyer_id
...
export async function createMessage({ fk_conversations_id, fk_users_id_sent, fk_users_id_received, content }) {
  ...
  const [rows] = await pool.query(
    'SELECT * FROM messages WHERE id_messages = ? LIMIT 1',
    [result.insertId]
  );
  return rows[0];
}
...
export async function markAsRead({ conversationId, userId }) {
  const [result] = await pool.query(
    `UPDATE messages SET read = true
     WHERE fk_conversations_id = ? AND fk_users_id_received = ? AND read = false`,
