...
export { getById, getPhotos };

/**
 * Listado público con filtros y paginación.
 * Solo devuelve items con conservation_status = 'published'
 * a menos que se filtre por vendedor concreto (para su propio perfil).
 */
export async function getPublished({ search, categoryId, location, minPrice, maxPrice, sellerId, page = 1, limit = 12 } = {}) {
  const params = [];
  const conditions = [];

  conditions.push(`i.conservation_status = 'published'`);

  if (sellerId) {
    conditions.push('i.fk_seller_id = ?');
  ...
  if (categoryId) {
    conditions.push('i.fk_categories_id = ?');
...
export async function createItem(data) {
  const { title, description, price, fk_categories_id, location, fk_seller_id } = data;
...
export async function updateItem(id, data) {
  const { title, description, price, fk_categories_id, location } = data;
  await pool.query(
...
export async function addPhotos(itemId, urls) {
  ...
  await pool.query(
    'INSERT INTO items_photos (fk_items_id, photo_url, `order`) VALUES ?',
    [values]
  );
...
export async function publishItem(id) {
  await pool.query(
    `UPDATE items SET conservation_status = 'published', publication_date = NOW(), item_update = NOW()
     WHERE id_items = ?`,
    [id]
  );
  return getById(id);
...
export async function markAsSold(id) {
  await pool.query(
    `UPDATE items SET conservation_status = 'sold', item_status = 'sold', item_update = NOW()
     WHERE id_items = ?`,
    [id]
  );
  return getById(id);
